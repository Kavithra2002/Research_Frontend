...
const g = globalThis as unknown as { __extractScanState?: ScanState };
if (!g.__extractScanState) {
  g.__extractScanState = {
    active: false,
    child: null,
    history: [],
    subscribers: new Set(),
...
const state = g.__extractScanState!;
...
function startScan(opts: ScanOptions) {
  if (state.active) return;
  state.active = true;
  state.history = [];

  const scriptDir = getScriptDir();
  const scriptPath = path.join(scriptDir, "Extract_newly_updated.py");
  ...
  const symbols = Array.isArray(opts.symbols)
    ? opts.symbols
        ...
        .filter((s) => s.length > 0)
    : [];
  const companies = Array.isArray(opts.companies)
    ? opts.companies
        .map((s) => (typeof s === "string" ? s.trim() : ""))
        .filter((s) => s.length > 0)
    : [];
  ...
  if (symbols.length > 0 || companies.length > 0) {
    try {
      const tmp = path.join(
        os.tmpdir(),
        `ambeon-scan-filter-${Date.now()}-${process.pid}.json`,
      );
      fs.writeFileSync(
        tmp,
        JSON.stringify({
      ...
      filterFile = tmp;
      args.push("--filter-file", tmp);
    } catch (err) {
      broadcast(
        JSON.stringify({
          type: "log",
          level: "warning",
          message: `Could not write filter file: ${
            err instanceof Error ? err.message : String(err)
          }. Falling back to args.`,
        }),
      );
      if (symbols.length > 0) {
  ...
  if (opts.groupName) {
    broadcast(
      JSON.stringify({
        type: "log",
        level: "info",
        message: `Scope: group "${opts.groupName}" (${symbols.length || companies.length} compan${
          (symbols.length || companies.length) === 1 ? "y" : "ies"
        })`,
      }),
    );
  }
  ...
  child.on("close", (code) => {
    ...
    stderrBuf = "";
    if (filterFile) {
      fs.unlink(filterFile, () => undefined);
      filterFile = null;
    }
    finalize(code ?? 0);
  });
...
export async function POST(request: NextRequest) {
  ...
  startScan({
    ...
    symbols: toStringArray(body.symbols),
