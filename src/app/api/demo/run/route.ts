...
const g = globalThis as unknown as { __demoRunState?: RunState };
if (!g.__demoRunState) {
  g.__demoRunState = {
    active: false,
    child: null,
...
async function startRun(items: DemoItem[], opts: StartOptions) {
  if (state.active) return;
  state.active = true;
  state.history = [];

  const scriptDir = getScriptDir();
  const scriptPath = path.join(scriptDir, "Demo_run.py");
...
export async function POST(request: NextRequest) {
  ...
  if (state.active) {
    // A run is already in flight — re-attach the caller to it.
