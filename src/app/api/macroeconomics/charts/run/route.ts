...
const g = globalThis as unknown as { __macChartsRunState?: RunState };
if (!g.__macChartsRunState) {
  g.__macChartsRunState = {
    active: false,
    child: null,
    history: [],
    subscribers: new Set(),
...
const state = g.__macChartsRunState!;
...
async function startRun() {
  ...
  const scriptPath = path.join(scriptDir, "Mac_Rep_script.py");
  const python = pythonCommand();
  const args = ["-u", scriptPath, "--json-events"];
...
export async function POST(request: NextRequest) {
  if (shouldProxyPythonToBackend()) {
    return proxyToBackend(request, "/api/macroeconomics/charts/run");
  }
