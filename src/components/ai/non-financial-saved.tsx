...
function reportLabel(report: NfReport) {
  return (
...
export function NonFinancialSaved() {
  ...
  // Drop a stale selection if its report disappeared after a refresh.
  React.useEffect(() => {
    if (!selection) return;
  ...
  return (
    <Card size="sm" className="flex max-h-[680px] min-h-[540px] flex-col overflow-hidden py-0">
      {/* ── Header ─────────────────────────────────────────────── */}
        ...
        <div className="flex min-w-0 items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <p className="truncate text-[11px] text-muted-foreground">
              Every company stored in the database with extracted non-financial
              data points.
            </p>
          </div>
        </div>
            ...
            <div className="flex flex-1 items-center justify-center p-6 text-center">
              <div className="max-w-xs">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  Generate a non-financial briefing on the{" "}
                  <strong>Generate</strong> tab to capture data into the
                  database, then refresh to browse it here.
                </p>
              </div>
            </div>
          ...
          {!selection ? (
            <div className="flex flex-1 items-center justify-center p-10 text-center">
              <div className="max-w-sm">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Pick a company and a report from the list to view its captured
            ...
            <div className="flex min-h-0 min-w-0 flex-1 flex-col">
              <div className="flex flex-wrap items-center gap-2 border-b bg-background/60 px-4 py-2.5">
                ...
                {formatDate(
                  (data?.meta?.generated_at as string | undefined) ??
                    selection.report.extractedAt,
                ) ? (
              ...
              <div className="min-h-0 min-w-0 flex-1 overflow-auto">
                <DataView data={data} loading={dataLoading} error={dataError} />
              </div>
            </div>
          )}
        </div>
  ...
  return (
    <div className="flex flex-col gap-4 p-4">
      ...
      {categories.length === 0 ? (
        ...
        categories.map((cat) => (
          <CategoryBlock key={cat.key} category={cat} showMissing={showMissing} />
        ))
      )}
    </div>
  );
