...
import { sendTuckChat, type TuckMessage } from "@/lib/tuck";
import {
  deleteChatSession,
  getChatSession,
  listChatSessions,
...
const AGENT = "tuck";
const TUCK_AVATAR = "/img/tuck-avatar.png";
...
export function TuckAgentPanel({ open, onOpenChange }: TuckAgentPanelProps) {
  ...
  async function send(text: string) {
    ...
    const history: TuckMessage[] = nextTurns.map((t) => ({
      role: t.role,
      content: t.text,
    }));
    ...
    try {
      const res = await sendTuckChat(history, controller.signal);
          ...
          <div className="flex items-center gap-3">
            <div className="relative">
              ...
              <Avatar size="lg" className="relative ring-2 ring-sky-500/40">
                <AvatarImage src={TUCK_AVATAR} alt="Tuck" />
            ...
            <div className="flex min-w-0 flex-1 flex-col">
              ...
              <SheetDescription className="flex items-center gap-1 text-xs">
                ...
                CSE market data · Financial report figures
              </SheetDescription>
            </div>
        ...
        {exportable && (
          <div className="mt-2 flex justify-end border-t border-border/60 pt-1.5">
            <button
              ...
              onClick={async () => {
                const avatarDataUrl =
                  (await loadImageDataUrl("/img/tuck-avatar.png")) ?? undefined;
